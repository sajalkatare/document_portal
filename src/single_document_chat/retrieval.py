...
class ConversationalRAG:
    def __init__(self, session_id: str, retriever):
        ...
        try:
            ...
            # Step 1: Contextualize question (history-aware)
            contextualize_chain = (
                {
            ...
            self.history_aware_retriever = contextualize_chain | self.retriever
            self.log.info("Created history-aware retriever", session_id=session_id)


            # Optional: format documents (HIGHLY recommended)
            ...
            self.qa_chain = (
                {
                    "context": self.history_aware_retriever | format_docs,
                    "input": lambda x: x["input"],
                    "chat_history": lambda x: x["chat_history"],
                }
                | self.qa_prompt
    ...
    def invoke(self, user_input: str) -> str:
        try:
            ...
            answer = response if response else "No answer."
